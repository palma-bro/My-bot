...
from telegram.ext import ApplicationBuilder, CommandHandler, CallbackQueryHandler, ContextTyp
...
"running": False,
"chat_id": None,
"min_score": 75, "interval": 15, "notified": {}, "symbols": [], # минимальный % уверенности для сигнала
# минуты между проверками
# {symbol: last_signal} чтобы не спамить
...
if result["score"] >= state["min_score"] and result["signal"] != "НЕЙТРАЛ
...
reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton(" Назад", callback_dat
)
elif data == "back":
await query.message.edit_text(
" *Bybit Futures Signal Bot*",
